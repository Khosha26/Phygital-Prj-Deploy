...
(function(){
  ...
  function init(){
    ensureBg(); wireChrome(); reveal(); injectFab();
    bindRipples(document); bindCounts(document);
    // accept parent ping
    window.addEventListener('message',function(e){ if(e&&e.data==='hm:screen-show'){ var s=$('.scr'); if(s){ s.classList.remove('ready'); void s.offsetWidth; reveal(); } } });
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',init); else init();

  window.HMKit={ back:back, openFloat:openFloat, closeFloat:closeFloat, lightbox:lightbox, swipe:swipe,
    countUp:countUp, ripple:rippleOn, bindRipples:bindRipples, reveal:reveal };
})();
