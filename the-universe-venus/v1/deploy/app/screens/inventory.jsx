...
function Inventory(){
  ...
  return (
    <div style={{position:'absolute', inset:0, background:'transparent', color:'var(--ink)', overflow:'hidden'}}>
      ...
      {/* ── UNIVERSE / VENUS LOCKUP ── */}
      <div style={{position:'absolute', top:54, right:46, zIndex:5, display:'flex', alignItems:'center', gap:22, animation:'invHeadIn 600ms cubic-bezier(0.22,1,0.36,1) both'}}>
      ...
      <div style={{position:'absolute', top:168, right:46, zIndex:5, display:'flex', gap:4, padding:5, borderRadius:100, ...invGlass}}>
        {[['all','ALL'],['available','AVAILABLE']].map(([id,lbl])=>(
          <button key={id} onClick={()=>setFilter(id)} className="mono" style={{
            padding:`${10*dens}px ${22*dens}px`, borderRadius:100, cursor:'pointer', border:'none',
            fontSize:12*dens, letterSpacing:'0.2em', transition:'all .2s',
            background: filter===id ? 'linear-gradient(180deg, #d8b873, #c9a05e)' : 'transparent',
            color: filter===id ? '#1a130a' : 'var(--slate)', fontWeight: filter===id?800:600,
          }}>{lbl}</button>
        ))}
      </div>
