// Splash — cinematic title sequence:
// ignition → cosmos → particles converge to LOGO IMAGE pixel positions →
// real client logo materialises over the constellation → subtitle → venus credit → warp out.
// Particles sample the actual `universe-logo-transparent.png` so the constellation
// traces the real client logo silhouette before the image itself fades in.

function ensureSplashKeys() {
  if (document.getElementById('splash-keys')) return;
  ...
  s.textContent = `
    ...
    @keyframes spDotBreathe {
      0%,100% { transform: scale(1);    filter: drop-shadow(0 0 14px rgba(232,216,179,0.85)); }
      50%     { transform: scale(1.18); filter: drop-shadow(0 0 28px rgba(232,216,179,1)); }
    }
  `;
...
function Splash() {
  ...
  // Runs once on mount; falls back to a circular pattern if image isn't loaded yet.
  ...
  React.useEffect(() => {
    ...
    img.onload = () => {
      ...
      const stride = 7;     // sample every 7px → ~200-260 targets
      const out = [];
      for (let y = 0; y < cv.height; y += stride) {
        for (let x = 0; x < cv.width; x += stride) {
          const a = data[(y * cv.width + x) * 4 + 3];
          if (a > 180) {
            // map image pixel → canvas position inside logo bbox
            out.push({
      ...
      // shuffle so particle assignments aren't grid-aligned
  ...
  React.useEffect(() => {
    const id = setTimeout(() => { setNavigated(true); navigate('home'); }, 7800);
  ...
  const convergeP  = clamp((t - 1.8) / 1.4);            // 1.8 → 3.2  pull to logo silhouette
  const bracketsP  = clamp((t - 2.2) / 0.6) * (1 - clamp((t - 3.6)/0.6));
  const ringsP     = clamp((t - 2.6) / 1.0);
  const logoP      = clamp((t - 3.4) / 1.2);            // 3.4 → 4.6  real logo fades in over particles
  const dispersP   = clamp((t - 4.2) / 0.9);            // 4.2 → 5.1  particles dissolve outward
  const subP       = clamp((t - 4.7) / 0.9);            // 4.7 → 5.6  subtitle typewriter
  const credP      = clamp((t - 5.4) / 1.0);            // 5.4 → 6.4  venus credit
  const warpP      = clamp((t - 7.1) / 0.7);            // 7.1 → 7.8  warp exit
  const fade       = navigated ? clamp((t - 7.3) / 0.5) : 0;

  // ---------- particle field ----------
  const N = 220;     // enough density to read the U silhouette
  ...
  // current target per particle: sampled logo pixel if available, else fallback ring
  const targetFor = (p, i) => {
  ...
  const positions = particles.map((p, i) => {
    ...
    if (t < 1.8) {
      ...
      // gentle micro-jitter while logo image fades in
      cx = tgt.x + Math.sin(t*1.6 + p.seed) * 1.6;
      cy = tgt.y + Math.cos(t*1.4 + p.seed) * 1.6;
    } else {
      // dispersal outward as image takes over
  ...
  // particle opacity fades as the real logo image takes over
  ...
  return (
    <div style={{
      ...
      background:'radial-gradient(ellipse 70% 60% at 50% 48%, #0d0a06 0%, #050402 70%, #000 100%)',
      color:'#f5f1e8', overflow:'hidden',
      ...
      <div style={{position:'absolute', inset:0, transform:`scale(${warpScale})`, transformOrigin:'50% 49%', transition:'transform 60ms linear'}}>
        ...
        {/* vignette */}
        <div style={{position:'absolute', inset:0, background:'radial-gradient(circle at 50% 50%, transparent 30%, rgba(0,0,0,0.55) 100%)', pointerEvents:'none'}}/>

        {/* gold core glow that intensifies as logo materialises */}
        <div style={{
          position:'absolute', inset:0,
          background:`radial-gradient(ellipse 38% 32% at 50% 47%, rgba(216,181,115,${0.05 + 0.22*logoP}), transparent 70%)`,
          pointerEvents:'none',
        }}/>
        ...
        {t < 0.7 && (
          <div style={{
            ...
            borderRadius:'50%', background:'#ead7a8',
            opacity: ignite,
            animation: 'spDotBreathe 0.8s ease-in-out infinite',
            boxShadow:'0 0 60px 10px rgba(232,216,179,0.65)',
          }}/>
        )}

        {/* bloom flash at ignition */}
        <div style={{
          position:'absolute', inset:0,
          background:'radial-gradient(circle at 50% 47%, rgba(245,228,180,0.85) 0%, rgba(216,181,115,0.45) 25%, transparent 55%)',
        ...
        <div style={{
          ...
          background:'linear-gradient(90deg, transparent, rgba(216,181,115,0.55) 18%, rgba(232,216,179,0.85) 50%, rgba(216,181,115,0.55) 82%, transparent)',
          opacity: 0.7 - 0.5*ringsP,
          boxShadow:'0 0 10px rgba(216,181,115,0.45)',
          pointerEvents:'none',
        }}/>
        ...
        <svg viewBox="0 0 2560 1600" preserveAspectRatio="xMidYMid slice"
          ...
          {/* near-neighbour web — only render when particles are still spread out */}
          {t < 3.4 && positions.map((a,i) => positions.slice(i+1, i+12).map((b,j) => {
            ...
            return <line key={`${i}-${j}`} x1={a.x} y1={a.y} x2={b.x} y2={b.y}
                         stroke="#c9a05e" strokeOpacity={0.32*(1-d/110)} strokeWidth="0.7"/>;
          }))}
          {streaks.map((s,i) => (
            <line key={`s${i}`} x1={s.x1} y1={s.y1} x2={s.x2} y2={s.y2}
                  stroke="#ead7a8" strokeOpacity={s.op} strokeWidth="1.2"/>
          ))}
          {positions.map((p,i) => (
          ...
          {[0,1,2].map(i => {
            ...
            return (
              <g key={i} style={{transformOrigin:`${LOGO_CX}px ${LOGO_CY}px`, animation:`${i%2 ? 'spOrbitSpinRev' : 'spOrbitSpin'} ${48 + i*16}s linear infinite`}}>
                <circle cx={LOGO_CX} cy={LOGO_CY} r={radius} fill="none" stroke="#c9a05e" strokeWidth="0.5" strokeOpacity="0.18"/>
                {Array.from({length: ticks}).map((_,k) => {
                  const a = (k / ticks) * Math.PI * 2;
                  const major = k % 10 === 0;
                  const inner = radius - (major ? 11 : 5);
                  ...
                  return <line key={k}
                    ...
                    stroke="#c9a05e" strokeWidth={major ? 1 : 0.5}
        ...
        <svg viewBox="0 0 2560 1600" style={{position:'absolute', inset:0, width:'100%', height:'100%', pointerEvents:'none'}}>
          {(() => {
            const rW = LOGO_W/2 + 60;
            const rH = LOGO_H/2 + 80;
            const tip = 36 * bracketsP;
            const op = bracketsP;
            const corners = [
            ...
            return corners.map(([x,y], i) => {
              ...
              return (
                <g key={i} opacity={op}>
                  <line x1={x} y1={y} x2={x + sx*tip} y2={y} stroke="#c9a05e" strokeWidth="1.6"/>
                  <line x1={x} y1={y} x2={x} y2={y + sy*tip} stroke="#c9a05e" strokeWidth="1.6"/>
                  <circle cx={x} cy={y} r="3" fill="#c9a05e"/>
                </g>
              );
            });
          })()}
        ...
        {/* === REAL LOGO IMAGE — fades in over the converged constellation === */}
        <div style={{
          position:'absolute',
          left: LOGO_LFT, top: LOGO_TOP,
          width: LOGO_W, height: LOGO_H,
          opacity: logoP,
          filter: `drop-shadow(0 0 ${28 * logoP}px rgba(232,216,179,${0.55*logoP}))`,
          transform: `scale(${0.96 + 0.04*logoP})`,
          transformOrigin: '50% 50%',
          transition: 'opacity 220ms ease, filter 220ms ease',
          pointerEvents:'none',
        }}>
          <UniverseLogoFull size={LOGO_W} progress={1}/>
        </div>

        {/* subtitle below the logo */}
        <div style={{
          position:'absolute', left:0, right:0,
          top: LOGO_TOP + LOGO_H + 36,
          display:'flex', justifyContent:'center',
          opacity: clamp(subP*1.3),
        }}>
          <div className="mono" style={{fontSize:18, letterSpacing:'0.42em', color:'rgba(245,241,232,0.65)', textTransform:'uppercase'}}>
            {tw('NEHRU NAGAR · AHMEDABAD · MMXXVI', subP)}
            <span style={{
              display:'inline-block', width:'0.55em', marginLeft:6,
              borderBottom:'1px solid #ead7a8',
        ...
        <div style={{
          ...
          <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:14}}>
            <div style={{height:1, width:60, background:'rgba(245,241,232,0.28)'}}/>
            <div className="mono" style={{fontSize:13, letterSpacing:'0.4em', color:'rgba(245,241,232,0.5)', textTransform:'uppercase'}}>
              A Venus Group of Companies project
            </div>
            <VenusWordmark size={28} color="#d63b50"/>
            <div className="serif" style={{fontSize:18, fontStyle:'italic', color:'rgba(245,241,232,0.55)'}}>
        ...
        {(() => {
          ...
          const baseSty = {fontSize:13, letterSpacing:'0.32em', color:'rgba(245,241,232,0.5)', textTransform:'uppercase', whiteSpace:'nowrap'};
          return (
            <>
              <div style={{position:'absolute', top: 48, left: 60, opacity: hudP, display:'flex', alignItems:'center', gap:14}}>
                <div style={{width:8, height:8, background:'#c9a05e',
                             animation:'spHudPulse 1.6s ease-in-out infinite',
                             boxShadow:'0 0 8px #c9a05e'}}/>
                <span className="mono" style={baseSty}>{tw(tlText, hudP)}</span>
              </div>
              <div style={{position:'absolute', top: 48, right: 60, opacity: hudP, display:'flex', alignItems:'center', gap:14}}>
                <span className="mono" style={baseSty}>{tw(trText, hudP)}</span>
                <div style={{width:8, height:8, border:'1px solid #c9a05e', borderRadius:'50%',
                             animation:'spHudPulse 1.6s ease-in-out infinite'}}/>
              </div>
              <div style={{position:'absolute', bottom: 50, left: 60, opacity: hudP, display:'flex', alignItems:'center', gap:14}}>
      ...
      <div style={{position:'absolute', bottom: 48, right: 60, opacity: clamp((t-1.0)/0.6)}}>
        <Magnetic radius={130} strength={0.32}>
          <ShimmerSweep speed={1100} color="rgba(232,216,179,0.45)">
            <button onClick={() => { setNavigated(true); navigate('home'); }} className="mono" style={{
              background:'transparent', border:'1px solid rgba(201,160,94,0.55)',
              color:'#c9a05e', padding:'14px 30px', borderRadius:100,
              fontSize:15, letterSpacing:'0.24em', textTransform:'uppercase',
              cursor:'pointer',
              transition:'background 240ms ease, border-color 240ms ease, color 240ms ease',
            }}
              onMouseEnter={ev=>{ ev.currentTarget.style.background='rgba(201,160,94,0.12)'; ev.currentTarget.style.borderColor='#c9a05e'; }}
              onMouseLeave={ev=>{ ev.currentTarget.style.background='transparent';            ev.currentTarget.style.borderColor='rgba(201,160,94,0.55)'; }}
