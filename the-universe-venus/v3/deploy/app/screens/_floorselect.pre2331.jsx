    ...
    <div style={{position:'absolute', inset:0, background:'#f6efe1', color:'var(--ink)', overflow:'hidden'}}>
      ...
      <div style={{position:'absolute', left:72, bottom:66, zIndex:6, display:'flex', alignItems:'center', gap:30}}>
        <div style={{display:'flex', alignItems:'center', gap:13}}>
          ...
          <span className="mono" style={{fontSize:14, letterSpacing:'0.32em', color:'var(--gold-deep)'}}>{tower.type.toUpperCase()}</span>
        </div>
        ...
        return (
          <div style={{position:'absolute', right:948, top:'50%', transform:'translateY(-50%)', zIndex:8,
            ...
            <div style={{display:'flex', alignItems:'center', gap:11}}>
              ...
              <span className="mono" style={{fontSize:13, letterSpacing:'0.28em', color:'var(--gold-deep)'}}>SELECTED ON MASTER PLAN</span>
            </div>
            ...
            <div style={{textAlign:'center'}}>
              ...
              <div className="mono" style={{fontSize:12.5, letterSpacing:'0.2em', color:'var(--slate)', marginTop:7}}>{tower.cluster.toUpperCase()}</div>
            </div>
          </div>
        );
      })()}
      ...
      <div style={{position:'absolute', top:206, right:88, bottom:90, width:820, zIndex:7,
        ...
        <div style={{padding:'26px 30px 0'}}>
          <div style={{display:'flex', alignItems:'center', justifyContent:'space-between'}}>
            <span className="mono" style={{fontSize:15*dens, letterSpacing:'0.30em', color:'var(--slate)'}}>SELECT A FLOOR</span>
            <div style={{display:'flex', alignItems:'center', gap:18}}>
              <span className="mono" style={{fontSize:14*dens, letterSpacing:'0.18em', color:'var(--gold-deep)'}}>{floorData.length} LEVELS</span>
          ...
          <div style={{padding:'18px 24px 6px'}}>
            <button className="fs-hero" onClick={()=>navigate(`floor/${towerId}/${ph.floor}`)}
              ...
              <div>
                ...
                <div className="mono" style={{fontSize:13*dens, letterSpacing:'0.22em', color:'var(--slate)', marginTop:8}}>{ph.total} {ph.total===1?'HOME':'HOMES'} · DUPLEX</div>
              </div>
              ...
              return (
                <button key={fl.floor} className="fs-row"
                  ...
                  <div style={{minWidth:0}}>
                    ...
                    <div className="mono" style={{fontSize:11.5*dens, letterSpacing:'0.16em', color:'var(--slate)', marginTop:5, whiteSpace:'nowrap'}}>{fl.total} {fl.total===1?'HOME':'HOMES'}</div>
                  </div>
                  {/* status pill */}
                  <span style={{display:'inline-flex', alignItems:'center', gap:9, padding:`${8*dens}px ${15*dens}px`, borderRadius:105,
                    ...
                    <span className="mono" style={{fontSize:12.5*dens, letterSpacing:'0.10em', color: st.color, fontVariantNumeric:'tabular-nums'}}>{txt}</span>
                  </span>
                  {/* chevron */}
        ...
        <div style={{padding:'14px 30px 20px', borderTop:'1px solid var(--line)', display:'flex', alignItems:'center', justifyContent:'center', gap:30}}>
          {[STATUS.available, STATUS.hold, STATUS.sold].map(s => (
            <div key={s.key} style={{display:'flex', alignItems:'center', gap:9}}>
              ...
              <span className="mono" style={{fontSize:12.5*dens, letterSpacing:'0.14em', color:'var(--graphite)'}}>{s.label}</span>
            </div>
          ))}
        </div>
      </div>
...
function CapStat({ n, label }) {
  return (
    <div style={{textAlign:'left'}}>
      ...
      <div className="mono" style={{fontSize:12, letterSpacing:'0.22em', color:'var(--slate)', marginTop:5}}>{label}</div>
    </div>
  );
}
