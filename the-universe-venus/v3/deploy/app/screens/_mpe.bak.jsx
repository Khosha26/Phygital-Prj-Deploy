  ...
  return (
    <div style={{ position:'absolute', inset:0, background:'transparent', overflow:'visible',
      ...
      <div style={{ position:'absolute', top:250, bottom:44, left:72, zIndex:24,
        ...
        transform: panelOpen ? 'translateX(0)' : 'translateX(-576px)',
        transition:'transform 460ms cubic-bezier(0.22,1,0.36,1)' }}>

        <div style={{ width:520, display:'flex', flexDirection:'column', borderRadius:26, overflow:'hidden', pointerEvents:'auto',
          ...MPX_GLASS, animation:'mpxFade 420ms ease both' }}>

          {/* search + filter */}
          <div style={{ padding:'24px 24px 16px', display:'flex', gap:12 }}>
            <div style={{ flex:1, display:'flex', alignItems:'center', gap:12, padding:'0 18px', height:54, borderRadius:14,
              background:'rgba(255,255,255,0.72)', border:'1px solid rgba(176,138,63,0.28)' }}>
              <svg viewBox="0 0 24 24" width={20} height={20} fill="none" stroke="var(--slate)" strokeWidth="1.7" strokeLinecap="round"><circle cx="11" cy="11" r="7"/><path d="M20 20 L16.2 16.2"/></svg>
            ...
            <button aria-label="Filter" style={{ width:54, height:54, borderRadius:14, flexShrink:0, cursor:'pointer',
              ...
              <svg viewBox="0 0 24 24" width={22} height={22} fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M3 5 H21 M6 12 H18 M10 19 H14"/></svg>
            </button>
          </div>
          ...
          <div style={{ padding:'0 24px 16px', display:'flex', flexWrap:'wrap', gap:8 }}>
            {MPX_FILTERS.map(f=>{ const on=filter===f;
              return <button key={f} onClick={()=>setFilter(f)} className="mono" style={{ padding:'9px 15px', borderRadius:100, cursor:'pointer', fontSize:11.5, letterSpacing:'0.08em', fontWeight:on?700:600,
          ...
          {/* library label + count */}
          <div style={{ padding:'2px 26px 12px', display:'flex', alignItems:'baseline', justifyContent:'space-between',
            borderBottom:'1px solid rgba(176,138,63,0.2)' }}>
            <span className="mono" style={{ fontSize:12, letterSpacing:'0.28em', color:'var(--gold-deep)', fontWeight:700 }}>PLAN LIBRARY</span>
            <span className="mono" style={{ fontSize:12, letterSpacing:'0.18em', color:'var(--slate)' }}>{filtered.length} PLANS</span>
          </div>

          {/* scrollable plan list */}
          <div className="mpx-scroll" style={{ flex:1, overflowY:'auto', padding:'12px 16px' }}>
            {filtered.map(p=>{ const on=sel===p.id;
              return (
                <button key={p.id} onClick={()=>setSel(p.id)} style={{ width:'100%', textAlign:'left', cursor:'pointer',
                  display:'flex', alignItems:'center', gap:16, padding:'14px 16px', marginBottom:9, borderRadius:15, position:'relative',
                  background: on?'linear-gradient(180deg, rgba(234,215,168,0.55), rgba(201,160,94,0.22))':'rgba(255,255,255,0.5)',
                  border: on?'1px solid rgba(176,138,63,0.55)':'1px solid rgba(176,138,63,0.16)',
                  boxShadow: on?'0 8px 22px rgba(176,138,63,0.22)':'none',
                  transition:'all .18s' }}>
                  {on && <span style={{ position:'absolute', left:0, top:13, bottom:13, width:4, borderRadius:4,
                  ...
                  <span style={{ width:48, height:48, borderRadius:'50%', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center',
                    background: on?'#fff':'rgba(243,240,230,0.9)', border:'1px solid rgba(176,138,63,0.34)', color:'var(--gold-deep)' }}>
                    <MpxGlyph type={p.icon} size={23}/>
                  </span>
                  <span style={{ flex:1, minWidth:0 }}>
                    <span className="serif" style={{ display:'block', fontSize:20, fontWeight:500, color:'var(--ink)', lineHeight:1.2,
                      whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{p.title}</span>
                    <span style={{ display:'flex', flexWrap:'wrap', gap:'4px 12px', marginTop:5 }}>
                      {p.tags.map(tag=>(
                  ...
                  {on
                    ? <span style={{ width:36, height:36, borderRadius:'50%', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center',
                        background:'linear-gradient(180deg, var(--gold) 0%, var(--gold-deep) 100%)', color:'#1a130a', boxShadow:'0 4px 12px rgba(176,138,63,0.4)' }}>
                        <svg viewBox="0 0 24 24" width={19} height={19} fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 6 L15 12 L9 18"/></svg>
                      </span>
                    : <span style={{ flexShrink:0, color:'rgba(120,98,54,0.4)', fontSize:25, lineHeight:1, marginRight:4 }}>›</span>}
                </button>
              );
            })}
            {filtered.length===0 && (
              <div style={{ padding:'48px 18px', textAlign:'center', color:'var(--slate)', fontSize:16 }}>No plans match your search.</div>
            )}
          </div>

          {/* footer */}
          <div style={{ padding:'14px 26px', borderTop:'1px solid rgba(176,138,63,0.2)', background:'rgba(255,255,255,0.32)' }}>
            <span className="mono" style={{ fontSize:11.5, letterSpacing:'0.14em', color:'var(--slate)' }}>
        ...
        <button onClick={()=>setPanelOpen(o=>!o)} aria-label={panelOpen?'Collapse panel':'Expand panel'}
          style={{ alignSelf:'center', marginLeft:-1, width:38, height:88, cursor:'pointer', pointerEvents:'auto', flexShrink:0,
