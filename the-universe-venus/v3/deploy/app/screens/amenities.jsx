...
const AM_DESC = {
  sport:     'Basketball, badminton, box cricket, pickleball and a jogging track for every age.',
  wellness:  'The Wellness Club across two levels — strength, yoga, crossfit, cardio & circuit — beside the podium pool.',
  social:    'Banquet, Café Universe, The Lounge, a Gen-z lounge, indoor games and guest rooms, interiors curated by HBA.',
  family:    'A Toddler’s Den and a dedicated children’s playground, safe by design.',
  landscape: 'Acres of landscaping by SWA: water feature, crystal & petal gardens, seating and accent groves.',
};
...
// ============================================================================
function AmCategoryRow({ g, i, active, onClick, dens = 0, level = 'All' }) {
  const Icon = AM_ICONS[g.id] || AM_ICONS.sport;
  const t = useLoop();
  const iconSz = Math.round(30 + dens * 8);
  const shown = level === 'All' ? g.items.length : g.items.filter(it => it.level === level).length;
  const noneAtLevel = level !== 'All' && shown === 0;
  return (
...
function Amenities() {
  ...
  // ── LEVEL FILTER — genuinely filters the visible set (not just dimming) ──
  const viewItems = (level === 'All') ? items : items.filter(it => it.level === level);
  const safeIdx = viewItems.length ? (((subIdx % viewItems.length) + viewItems.length) % viewItems.length) : 0;
  const item  = viewItems[safeIdx] || null;
  ...
  const pickSub = (i) => { const n = viewItems.length || 1; setSubIdx(((i % n) + n) % n); };
  // Pick a level: reset to first match, and if the current wing has none at
  // that level, jump to the first wing that does — so the filter always shows
  // something and never strands the user on an empty category.
  const chooseLevel = (lv) => {
    setLevel(lv); setSubIdx(0);
    if (lv !== 'All' && !groups[catIdx].items.some(it => it.level === lv)) {
      const j = groups.findIndex(g => g.items.some(it => it.level === lv));
      if (j >= 0) setCatIdx(j);
    }
  };
  ...
  }, [subIdx, catIdx, level, viewItems.length]);
      ...
      <div style={{position:'absolute', top:300, left:72, right:72, bottom:56, display:'flex', gap:34}}>
        ...
        <div style={{
          ...
          <div style={{display:'flex', flexDirection:'column', justifyContent:'center', gap: Math.round(14 + dens*10), flex:1, minHeight:0}}>
            {groups.map((g, i) => (
              <AmCategoryRow key={g.id} g={g} i={i} active={i === catIdx} onClick={()=>pickCategory(i)} dens={dens} level={level}/>
            ))}
          </div>
          ...
          <div style={{marginTop: Math.round(26 + dens*10)}}>
            ...
            <LevelFilter level={level} setLevel={chooseLevel} dens={dens}/>
          </div>
        </div>
          ...
          <div style={{
            ...
            <div style={{position:'absolute', left:48, right:48, bottom:104, pointerEvents:'none'}}>
              <div className="mono" style={{fontSize:17, letterSpacing:'0.4em', color:'var(--gold-soft)'}}>
                {cat.label.toUpperCase()} · {pad2(viewItems.length ? safeIdx + 1 : 0)} OF {pad2(viewItems.length)}
              </div>
            ...
            <div style={{position:'absolute', left:0, right:0, bottom:34, display:'flex', justifyContent:'center'}}>
              <div style={{display:'flex', alignItems:'center', gap:22, padding:'10px 16px', borderRadius:999,
                ...
                <div className="mono" style={{fontSize:18, letterSpacing:'0.1em', color:'var(--ink)', fontVariantNumeric:'tabular-nums', whiteSpace:'nowrap', padding:'0 6px'}}>
                  {pad2(viewItems.length ? safeIdx + 1 : 0)}<span style={{color:'var(--gold-deep)', margin:'0 6px', opacity:0.7}}>/</span>{pad2(viewItems.length)}
                </div>
        ...
        <div style={{
          ...
          <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between', paddingBottom: Math.round(14+dens*4),
            ...
            <div className="mono" style={{fontSize: Math.round(17 + dens*3), letterSpacing:'0.2em', color:'var(--slate)', fontVariantNumeric:'tabular-nums'}}>
              {level === 'All'
                ? `${items.length} TOTAL`
                : `${viewItems.length} · ${(AM_LEVEL_LABEL[level] || level).toUpperCase()}`}
            </div>
          </div>
          ...
          {(() => {
            ...
            const needed = viewItems.length * thumbH + Math.max(0, viewItems.length - 1) * gap;
            ...
            if (viewItems.length === 0) {
              return (
                <div style={{flex:1, minHeight:0, display:'flex', alignItems:'center', justifyContent:'center', textAlign:'center', padding:'0 24px'}}>
                  <div className="mono" style={{fontSize: Math.round(17 + dens*3), letterSpacing:'0.2em', color:'var(--slate)', textTransform:'uppercase', lineHeight:1.7}}>
                    No {AM_LEVEL_LABEL[level] || level}<br/>amenities in this wing
                  </div>
                </div>
              );
            }
            return (
              <div ref={stackRef} className="am-stack" style={{flex:1, minHeight:0, overflowY:'auto', overflowX:'hidden',
                ...
                {viewItems.map((it, idx) => (
                  <div key={it.name + idx} data-active={idx === safeIdx ? '1' : '0'} style={{flexShrink:0}}>
                    <AmThumbCard
                      item={it} idx={idx}
                      active={idx === safeIdx}
                      dimmed={false}
