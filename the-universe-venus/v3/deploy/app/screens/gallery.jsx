...
function CategoryRow({ label, count, active, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      flex:'1 1 0', minHeight:96, maxHeight:150, padding:'0 30px', borderRadius:18,
    ...
    >
      <div className="mono" style={{
        flex:1, fontSize:21, letterSpacing:'0.24em', textTransform:'uppercase',
        color: active ? 'var(--ink)' : 'var(--graphite)', fontWeight: active ? 700 : 500,
      }}>{label}</div>
      <div className="mono" style={{
        fontSize:17, letterSpacing:'0.12em', fontVariantNumeric:'tabular-nums',
        padding:'7px 15px', borderRadius:12,
...
function ThumbCard({ item, idx, active, onClick }) {
  return (
    <button onClick={onClick} style={{
      position:'relative', width:'100%', height:178, flexShrink:0,
    ...
    >
      ...
      <div style={{position:'absolute', top:12, right:12, padding:'4px 11px', borderRadius:105,
        ...
        <span className="mono" style={{fontSize:17, letterSpacing:'0.22em', color:'var(--gold-soft)', textTransform:'uppercase'}}>{item.tag}</span>
      </div>
      {/* pano badge */}
      {isPano(item) && (
        <div style={{position:'absolute', top:12, left:12, padding:'4px 10px', borderRadius:105,
          background:'rgba(14,12,9,0.62)', border:'1px solid rgba(232,215,168,0.42)', backdropFilter:'blur(6px)'}}>
          <span className="mono" style={{fontSize:17, letterSpacing:'0.2em', color:'var(--gold-soft)', textTransform:'uppercase'}}>Pano</span>
        </div>
      )}
      {/* active check chip */}
      {active && (
      ...
      <div style={{position:'absolute', left:18, right:18, bottom:16, textAlign:'left'}}>
        <span className="serif" style={{fontSize:25, fontWeight:300, letterSpacing:'-0.01em',
...
function Gallery() {
  ensureGalleryKeys();
  const t = useLoop();
  // Canvas-height / TV density (matches Amenities so the left rail scales identically).
  ...
  const [pdfOpen, setPdfOpen] = React.useState(false); // brochure PDF viewer
  ...
  return (
    <div style={{position:'absolute', inset:0, background:'transparent', color:'var(--ink)', overflow:'hidden'}}>
      ...
      {/* ===== THREE-COLUMN STAGE ====================================== */}
      <div style={{position:'absolute', top:296, left:72, right:72, bottom:48, display:'flex', gap:34}}>

        {/* ---- LEFT EDITORIAL RAIL (spacing matched to Amenities) ---- */}
        <div style={{
          ...
          {/* Category list — centred group (matches Amenities' balanced rail) */}
          <div className="gal-stack" style={{display:'flex', flexDirection:'column', justifyContent:'center', gap: Math.round(14 + dens*10), flex:1, minHeight:0, overflowY:'auto'}}>
            {cats.map((c, i) => (
              <CategoryRow key={c} label={c} count={catCount(c)} active={i === catIdx} onClick={()=>pickCategory(i)}/>
            ))}
          </div>

          {/* Brochure PDF button — opens the full brochure in a fullscreen viewer */}
          <div style={{marginTop: Math.round(26 + dens*10)}}>
            <button onClick={()=>setPdfOpen(true)} className="mono" style={{
              display:'flex', alignItems:'center', justifyContent:'center', gap:14, width:'100%',
              padding:`${Math.round(18 + dens*6)}px 24px`, borderRadius:16, cursor:'pointer',
              border:'1px solid var(--gold-deep)',
              background:'linear-gradient(180deg, var(--gold) 0%, var(--gold-deep) 100%)',
              color:'#1a130a', fontSize: Math.round(16 + dens*3), letterSpacing:'0.22em', textTransform:'uppercase', fontWeight:700,
              boxShadow:'0 14px 30px -10px rgba(176,138,63,0.5), inset 0 1px 0 rgba(255,255,255,0.4)',
              ...
              <svg width={Math.round(20+dens*4)} height={Math.round(20+dens*4)} viewBox="0 0 24 24" fill="none" stroke="#1a130a" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div style={{
            ...
            {item && isPano(item) && (
              <div className="mono" style={{position:'absolute', top:30, left:30, padding:'8px 16px', borderRadius:105,
                ...
                fontSize:17, letterSpacing:'0.24em', color:'var(--gold-soft)', textTransform:'uppercase'}}>Panorama</div>
            )}
            ...
            {item && (
              <div style={{position:'absolute', left:48, right:48, bottom:104, pointerEvents:'none'}}>
                ...
                <div style={{display:'flex', alignItems:'center', gap:16, marginTop:16}}>
                  ...
                  <div style={{fontSize:20, color:'rgba(250,246,232,0.86)', fontStyle:'italic'}}>Tap to view full screen</div>
                </div>
              </div>
            )}
        ...
        <div style={{
          ...
          <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between', paddingBottom:16,
            ...
            <div className="mono" style={{fontSize:17, letterSpacing:'0.34em', color:'var(--gold-deep)', textTransform:'uppercase'}}>
              In this set
            </div>
            <div className="mono" style={{fontSize:17, letterSpacing:'0.18em', color:'var(--slate)', fontVariantNumeric:'tabular-nums'}}>
              {pad2(total)} TOTAL
            </div>
          </div>
...
// ============================================================================
// PdfViewer — fullscreen brochure viewer. Dark takeover with a header (title +
// Fullscreen + Close), then an <iframe> of the brochure PDF. ESC closes.
// ============================================================================
function pdfBtnStyle() {
  return { display:'flex', alignItems:'center', gap:10, cursor:'pointer',
    padding:'13px 24px', borderRadius:999, border:'1px solid rgba(232,215,168,0.4)',
    background:'rgba(255,255,255,0.06)', color:'var(--gold-soft)', fontSize:15, letterSpacing:'0.18em', textTransform:'uppercase' };
}
function PdfViewer({ onClose }) {
  const t = useLoop();
  const fadeIn = clamp(t/0.4);
  const wrapRef = React.useRef(null);
  const SRC = 'assets/docs/the-universe-brochure.pdf';
  React.useEffect(() => {
  ...
  return (
    <div data-crm="1" ref={wrapRef} style={{position:'absolute', inset:0, zIndex:90, display:'flex', flexDirection:'column',
      background:'#0c0a07', opacity:fadeIn, transition:'opacity 320ms ease'}}>
      <div style={{flexShrink:0, display:'flex', alignItems:'center', justifyContent:'space-between',
        padding:'26px 44px', borderBottom:'1px solid rgba(232,215,168,0.16)', background:'rgba(20,15,7,0.65)'}}>
        <div>
          <div className="mono" style={{fontSize:15, letterSpacing:'0.4em', color:'var(--gold-soft)', textTransform:'uppercase'}}>The Universe · Venus</div>
          <div className="serif" style={{fontSize:36, fontWeight:300, color:'#fff7e6', marginTop:5, lineHeight:1}}>The Brochure</div>
        </div>
        <div style={{display:'flex', alignItems:'center', gap:14}}>
          <button onClick={goFull} className="mono" style={pdfBtnStyle()}
            onMouseEnter={e=>e.currentTarget.style.background='rgba(232,215,168,0.14)'}
          ...
          <button onClick={onClose} className="mono" style={pdfBtnStyle()}
            onMouseEnter={e=>e.currentTarget.style.background='rgba(232,215,168,0.14)'}
            onMouseLeave={e=>e.currentTarget.style.background='rgba(255,255,255,0.06)'}>
      ...
      <div style={{flex:1, minHeight:0, background:'#1a1712'}}>
        <iframe src={SRC + '#view=FitH'} title="The Universe Brochure" style={{width:'100%', height:'100%', border:'none', display:'block'}}/>
      </div>
    </div>
  );
}
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <div style={{position:'absolute', left:50, bottom:50, color:'var(--ivory)', pointerEvents:'none', maxWidth:'70%'}}>
          <div className="mono" style={{fontSize:17, letterSpacing:'0.3em', opacity:0.9, color:'var(--gold-soft)'}}>{item.tag}</div>
        ...
        <div style={{position:'absolute', top:25, right:25, display:'flex', gap:11, alignItems:'center'}}>
          <div className="mono" style={{padding:'11px 19px', borderRadius:105, background:'rgba(245,241,232,0.1)', color:'var(--ivory)', fontSize:17, letterSpacing:'0.2em', backdropFilter:'blur(6px)'}}>
